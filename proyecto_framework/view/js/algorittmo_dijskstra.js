...
function generar() {
    var cost = $("#num_costo").val();
    if( parseFloat(cost) == 0 ){
        swal({
            type: 'info',
            title: 'Nota!!',
            text: "Debe ingresar el costo del galón",
        })
        return 0;
    }
    //Gif carga nodos
...
//console.log("cero");
                ...
                var distance = puntos_optimos.routes[0].legs[i].distance.value;
                ...
                route_optimal.push({nodo_inicio: start, distancia: distancia, nodo_fin: end,distance:distance});
...
function mostrar_rutas(nodos) {
    ...
    for (var i = 0; i < route_optimal.length; i++) {
        ...
        sum_distance = sum_distance + route_optimal[i].distance;
    ...
    for (var i = 0; i < route_optimal.length; i++) {
        ...
        html += '<td> = </td>';
        // html += '<td>('+ (parseFloat(route_optimal[i].distancia) / 60).toFixed(0) + ' min) ' + route_optimal[i].distancia + ' segundos</td>';
        html += '<td>'+ (parseFloat(route_optimal[i].distancia) / 60).toFixed(0) + ' min</td>';
        html += '</td>';
        html += '</tr>';
    }
    var min_total = 0;
    ...
    km_total = sum_distance / 1000;

    var costo_galon = $("#num_costo").val();
    costo_total = ((km_total * 0.26) * costo_galon).toFixed(2);

    html += '<tr>';
    html += '<td colspan=4 style="text-align:left">Distancia Total Min</td>';
    // html += '<td> (' + min_total.toFixed(0) + ' min)' + sum + ' segundos </td>';
    html += '<td>' + min_total.toFixed(0) + ' min</td>';
    html += '</tr>';
    html += '<tr>';
    html += '<td colspan=4 style="text-align:left">Distancia Total Km</td>';
    html += '<td>' + km_total.toFixed(2) + 'km</td>';
    html += '</tr>';
    html += '<tr>';
    html += '<td colspan=4 style="text-align:left">2.6 Galones cada 10 km</td>';
    html += '<td> 0.26 por 1km</td>';
    html += '</tr>';
    html += '<tr>';
    html += '<td colspan=4 style="text-align:left">COSTO TOTAL: </td>';
    html += '<td>S/.' + costo_total  +'</td>';
    html += '</tr>';
    html += '</tbody>';
    html += '</table>';
