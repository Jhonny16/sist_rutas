...
try {
    ...
    $cambio_password = $_POST["p_cambio_password"];
    ...
    if ($operacion == "agregar") {
        ...
        if($resultado == -1){
            Helper::imprimeJSON(203, "Este usuario ya existe", "");
        }else{
            $resultado = $objPersona->agregar($tipo_cliente, $area, $cargo, $usuario, $tipo_usuario, $password,$password_c, $password_t, $razon_social,$id_zona);
            if($resultado == 0){
                Helper::imprimeJSON(203, "No hubo resultados", "");
            }else{
                if ($resultado == 1) {
        ...
        $resultado = $objPersona->editar_persona($id_persona, $usuario, $tipo_usuario, $area, $cargo, $tipo_cliente, $new_password, $new_password_c, $new_password_t,$id_zona,$cambio_password);
        if ($resultado) {
